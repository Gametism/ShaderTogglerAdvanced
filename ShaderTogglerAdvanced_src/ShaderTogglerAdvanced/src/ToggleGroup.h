...
namespace ShaderToggler
{
 ...
 public:
  ...
  ToggleGroup makeDuplicate() const;
        const std::vector<finder::Signature>& getEffectFilters() const { return m_effectFilters; }
        bool addEffectFilter(const finder::Signature& rule)
        {
            if(!finder::valid(rule) || m_effectFilters.size()>=finder::maxRules ||
                std::find(m_effectFilters.begin(),m_effectFilters.end(),rule)!=m_effectFilters.end()) return false;
            m_effectFilters.push_back(rule);return true;
        }
        void removeEffectFilter(size_t index) { if(index<m_effectFilters.size()) m_effectFilters.erase(m_effectFilters.begin()+index); }
 ...
 private:
  ...
  std::unordered_set<uint32_t> m_computeShaderHashes;
        std::vector<finder::Signature> m_effectFilters;
 };
}
