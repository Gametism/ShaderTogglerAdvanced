...
//GT
	...
	void ToggleGroup::addTimedSuppressionKey(const KeyData& key)
	{
		...
		m_timedSuppressionKeys.push_back(key);
	}

	void ToggleGroup::setTimedSuppressionKeyAt(size_t index, const KeyData& key)
	{
		if (!key.isValid())
			return;

		if (index < m_timedSuppressionKeys.size())
		{
			m_timedSuppressionKeys[index] = key;
		}
		else if (index == m_timedSuppressionKeys.size())
		{
			addTimedSuppressionKey(key);
		}
	}

	void ToggleGroup::removeTimedSuppressionKeyAt(size_t index)
	{
		if (index >= m_timedSuppressionKeys.size())
			return;

		m_timedSuppressionKeys.erase(m_timedSuppressionKeys.begin() + static_cast<std::ptrdiff_t>(index));
	}

	void ToggleGroup::clearTimedSuppressionKeys()
	{
		m_timedSuppressionKeys.clear();
	}

	bool ToggleGroup::hasTimedSuppressionKeys() const
	{
		return !m_timedSuppressionKeys.empty();
	}

	size_t ToggleGroup::getTimedSuppressionKeyCount() const
	{
		return m_timedSuppressionKeys.size();
	}

	const KeyData& ToggleGroup::getTimedSuppressionKeyAt(size_t index) const
	{
		return m_timedSuppressionKeys.at(index);
	}

	std::string ToggleGroup::getTimedSuppressionKeyAsString(size_t index) const
	{
		return m_timedSuppressionKeys.at(index).toString();
	}

	const std::vector<KeyData>& ToggleGroup::getTimedSuppressionKeys() const
	{
		return m_timedSuppressionKeys;
	}

	void ToggleGroup::addAssignedTechniqueName(const std::string& name)
	{
		if (name.empty())
			return;

		m_assignedTechniqueNames.push_back(name);
	}

	void ToggleGroup::setAssignedTechniqueNameAt(size_t index, const std::string& name)
	{
		if (name.empty())
			return;

		if (index < m_assignedTechniqueNames.size())
		{
			m_assignedTechniqueNames[index] = name;
		}
		else if (index == m_assignedTechniqueNames.size())
		{
			addAssignedTechniqueName(name);
		}
	}

	void ToggleGroup::removeAssignedTechniqueNameAt(size_t index)
	{
		if (index >= m_assignedTechniqueNames.size())
			return;

		m_assignedTechniqueNames.erase(m_assignedTechniqueNames.begin() + static_cast<std::ptrdiff_t>(index));
	}

	void ToggleGroup::clearAssignedTechniqueNames()
	{
		m_assignedTechniqueNames.clear();
	}

	bool ToggleGroup::hasAssignedTechniqueNames() const
	{
		return !m_assignedTechniqueNames.empty();
	}

	size_t ToggleGroup::getAssignedTechniqueNameCount() const
	{
		return m_assignedTechniqueNames.size();
	}

	const std::string& ToggleGroup::getAssignedTechniqueNameAt(size_t index) const
	{
		return m_assignedTechniqueNames.at(index);
	}

	const std::vector<std::string>& ToggleGroup::getAssignedTechniqueNames() const
	{
		return m_assignedTechniqueNames;
	}

	ToggleGroup::AssignedTechniqueMode ToggleGroup::getAssignedTechniqueMode() const
	{
		return m_assignedTechniqueMode;
	}

	void ToggleGroup::setAssignedTechniqueMode(AssignedTechniqueMode mode)
	{
		m_assignedTechniqueMode = mode;
	}
...
//GT
		...
		const std::vector<uint32_t> timedSuppressionKeyValues = iniFile.GetArray("TimedSuppressionKeys", sectionRoot);
		for (const uint32_t keyValue : timedSuppressionKeyValues)
		{
		...
		const int assignedTechniqueCount = iniFile.GetInt("AssignedTechniqueCount", sectionRoot);
		if (assignedTechniqueCount != INT_MIN && assignedTechniqueCount > 0)
		{
			for (int i = 0; i < assignedTechniqueCount; ++i)
			{
				const std::string techniqueName = iniFile.GetValue("AssignedTechnique" + std::to_string(i), sectionRoot);
				if (!techniqueName.empty())
					addAssignedTechniqueName(techniqueName);
			}
		}

		const int assignedTechniqueModeValue = iniFile.GetInt("AssignedTechniqueMode", sectionRoot);
		if (assignedTechniqueModeValue != INT_MIN)
		{
			m_assignedTechniqueMode = assignedTechniqueModeFromInt(assignedTechniqueModeValue);
		}
		else
		{
			m_assignedTechniqueMode = AssignedTechniqueMode::InjectAtGroupShader;
...
//GT 
	void ToggleGroup::saveState(CDataFile& iniFile, int index, bool usingCustomFormat) const
	{
		...
		if (!m_timedSuppressionKeys.empty())
		{
		...
		iniFile.SetInt("AssignedTechniqueCount", static_cast<int>(m_assignedTechniqueNames.size()), "", sectionRoot);
		iniFile.SetInt("AssignedTechniqueMode", assignedTechniqueModeToInt(m_assignedTechniqueMode), "", sectionRoot);
		for (int i = 0; i < static_cast<int>(m_assignedTechniqueNames.size()); ++i)
		{
			iniFile.SetValue("AssignedTechnique" + std::to_string(i), m_assignedTechniqueNames[static_cast<size_t>(i)], "", sectionRoot);
		}
