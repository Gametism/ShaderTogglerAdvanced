...
namespace ShaderToggler::finder
{
    ...
    // API, kind, VS, PS, CS, three call dimensions, a stable pipeline key,
    // and optional remembered shader stage/hash (1=pixel, 2=vertex, 3=compute).
    // Never persist object pointers, resource handles or frame positions.
    using Signature = std::array<uint32_t,12>;
    using History = std::array<uint32_t,3>; // Last known pixel, vertex, compute hashes.
    ...
    inline bool valid(const Signature& s)
    {
        ...
        if(s[1]>=4) return s[7]==1 || (s[7]==2 && s[0]==4 && s[1]<=5); // 2: DX12 graphics with binding updates.
    ...
    struct Match
    {
        ...
        std::optional<Signature> candidate(const Signature& s,const History& history,bool original) const
        {
            ...
            {
                // Prefer compute context when several old shader matches overlap.
                // A saved result retains the same condition as its preview.
    ...
    {
    public:
        // Called under the state lock before a preview becomes visible to render
        // callbacks. Must not re-enter the engine. No trace from observe().
    ...
    private:
        ...
        void prepareBatch(const char* reason)
        {
            ...
            for(size_t i=0;i<count;++i)
            {
                ...
                // Never suppress multiple compute producers, or mix a producer
                // with graphics consumers in an automatic batch.
        ...
        bool advance(uint64_t now)
        {
            ...
            if((stage==Stage::Baseline || stage==Stage::Action) && now>=end)
            {
                if(stage==Stage::Baseline)
                {
                    ...
                    {
                        // Schedule from the current presentation, not a stale deadline
                        // after a pause, so the player always gets the full prompt.
    ...
    public:
        ...
        void setComputeCandidates(bool enabled)
        {
            ...
            // Changing the candidate scope must not immediately disable a newly
            // included compute shader. The next explicit preview starts testing.
        ...
        // The record callback uses its own deadline, even if presentation pauses.
        bool observe(uintptr_t device,const Signature& key,uint64_t now,bool canSkip=true,const History& history={})
        {
            if(isolates(device))
            {
                ...
                // Kept whole-shader groups are applied by the normal draw callback.
                // Exclude their work from both capture and candidate confirmation.
                ...
                if(now>=start && now<end && (stage==Stage::BaselineDelay || stage==Stage::Baseline || stage==Stage::ActionDelay || stage==Stage::Action || stage==Stage::FrozenDelay || stage==Stage::FrozenCapture))
                {
                    ...
                    if(!selected) { /* Outside the source toggle's matching context. */ }
        ...
        bool answer(bool gone)
        {
            ...
            // An individual result is confirmed/saved by the caller, never as a batch.
