...
namespace ShaderToggler
{
	...
	bool ToggleGroup::isActive m_active; }
	...
	const std::unordered_set<uint32_t>& ToggleGroup::() const { return m_pixelShaderHashes; }
	...
	void ToggleGroup::loadState(CDataFile& iniFile, int index, bool usingCustomFormat)
	{
		...
		if (index < 0)
		{
			...
			for (int i = 0; i < amount; i++)
			{
				uint32_t hash = iniFile.GetUI + std::to_string(i), "ComputeShaders");
				if (hash != UINT_MAX)
		...
		const std::string holdInvertedValue = iniFile.GetValue, sectionRoot);
		if (!holdInvertedValue.empty())
			m_holdInverted = iniFile.GetBool, sectionRoot);
		else
			m_holdInverted = false;
	...
	{
		const std::string prefix = usingCustomFormat ? 
		const std::string sectionRoot = prefix + std::to_string(index);
		const std::string vertexHashesCategory = sectionRoot 
		const std::string pixelHashesCategory = sectionRoot 
		const std::string computeHashesCategory = sectionRoot 
