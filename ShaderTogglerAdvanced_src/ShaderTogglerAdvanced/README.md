...
| **ShaderToggler.ini.bak** | Previous configuration for manual recovery |
...
Expandable sections use outlined, full-width blue bars with clear arrows. Click anywhere on a bar to open or close it. This applies to **HUD visibility and shader selection**, manual controls, saved filters, advanced colour, timed controls and settings.

Finder controls show the current status and actions without repeating the shortcuts already printed on the buttons. Longer instructions are under **How to use the finder** and **How to use Smart disable**, or in **(?)** tooltips. The floating Finder HUD still shows the shortcuts during gameplay. The HUD group checklist scrolls when needed, and the Help tab groups its information into clear sections with a compact hunting-shortcut table.

Each group occupies one compact row:

- A dotted drag handle.
- A clickable, coloured **ON/OFF** button.
...
Click the **ON/OFF** button at the left of any group to switch it immediately, including refined groups and groups without a hotkey. **ON** means the group is disabling its selected effects; **OFF** allows them. Hover and pressed colours make the button easy to identify.

For hold or timed groups, the menu choice stays in effect until the next change in that group's assigned controls. A menu click cancels its current startup timer without changing its saved settings. Menu clicks do not change **Active at startup**.

While all groups are suspended, click **Restore** first. During Finder sessions, use **HUD visibility and shader selection**, or stop Finder before switching normal groups.
...
Saves write and flush a temporary file before replacing the live INI. When a configuration already exists, it is copied to **ShaderToggler.ini.bak** before replacement. If writing, backing up or replacing fails, the live configuration stays intact and the overlay reports a save failure. The log identifies the failed step. To restore a backup, close the game, keep a copy of the current INI, and copy ShaderToggler.ini.bak to ShaderToggler.ini.

If **Keep and finish** or **Keep and test more** cannot save, Finder keeps the same preview open and removes the uncommitted filter from the group. Resolve the file-access or disk-space issue, then use the same button to retry.

Deleting another group preserves the current hunt, shader selection and live marks. Deleting the group being hunted ends that hunt. Finder closes if a deleted group is its destination, source or a group selected to remain applied.
