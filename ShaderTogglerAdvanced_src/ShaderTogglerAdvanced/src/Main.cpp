...
// Exported data keeps this notice in optimized ASI/DLL/add-on builds.
...
struct __declspec(uuid("038B03AA-4C75-443B-A695-752D80797037")) CommandListDataContainer
{
    ...
    // Original ShaderToggler matches the last known handle of each type,
    // including across unknown bindings and graphics/compute PSO changes.
...
static int g_finderMode = 0; // 0: live comparison, 1: paused scene (next session).
...
static int g_uiScalePercent=0; // 0: auto; 100-250: multiplier of the ReShade font size.
static char g_groupSearch[160] = {};
static int g_groupListFilter=0; // All, refined, active.
static int g_standaloneFinderDestination=-1;
static int g_uiSaveResult=0; // Last completed save: 0 unknown, 1 success, -1 failure.
...
static float addonUiScale(float fontSize,const ImVec2& displaySize,int percent)
{
    ...
    // Readable target sizes for 4K/1440p/1080p, without multiplying an already
    // larger ReShade font again. Keep the same bounded range as manual sizing.
...
// Use the host's modern font stack where available; older hosts retain the
// public window-scale path. Both are strictly scoped to this add-on's drawing.
...
// Apply spacing only while drawing this add-on. ReShade owns the shared theme.
...
// Larger navigation and panel actions without expanding the compact group rows.
// Use public style pushes so the settings also reach newer ReShade ImGui hosts.
...
// Framed, full-width disclosure bars distinguish controls from explanatory text.
// Tree nodes retain their ID stack and TreePop pairing for the existing editors.
...
// Keep action rows usable when the ReShade panel is narrow or the font is large.
...
bool saveShaderTogglerIniFile()
{
	...
	const auto result=ini_save::save(g_iniFileName,files,[&](const std::filesystem::path& temporary)
	{
		...
		// CDataFile normally saves dirty data from its destructor. Disable that
		// retry before the temporary file is removed or committed.
		iniFile.Clear();
...
{
    // Edges are tracked locally: holding a key must never accept several tests.
    ...
    // Still consume the edges above while typing, so a held key is not replayed
    // as an answer when the overlay releases keyboard focus.
...
static void onBindPipeline(command_list* commandList, pipeline_stage stages, pipeline pipelineHandle)
{
    ...
    if (smart::dx12::supported(commandList->get_device()))
    {
        ...
        // D3D12 has one current PSO shared by graphics and compute.
	...
	// Only the stages in this binding changed. A null/unknown shader in an
	// affected stage must clear the previous handle instead of leaving it active.
...
bool blockDrawCallForCommandList(command_list* commandList, SmartDrawRequest* replacement = nullptr)
{
    ...
    if (originalMatching)
    {
        ...
        // Historical matches restore old toggles; only a verified current pixel
        // shader can receive a Smart colour replacement.
        if (computeBlocked || (pixelBlocked && originalPixel != pixelHash))
        { smart::restore(commandList->get_device());return true; }
    }
    for (const auto& group : g_toggleGroups)
    {
        // The hunting managers own the edited group's live selection.
    ...
    if ((other || dx12 || modern || smart::native(device)) && preview.hash && preview.hash == pixelHash && preview.hash == huntedHash && preview.original)
        pixelBlocked = false;
    // An independently hidden vertex shader still wins. Original compute
    // matches were handled above; strict matching ignores compute on draws.
    ...
    if (other || dx12 || modern || smart::native(device))
    {
        ...
        if (choice && choice->method != smart::Method::Skip)
        {
            if (smart::other::isVK(device))
            {
                ...
                return false; // Preserve the native draw, including indirect count buffers.
            }
            if (dx12)
            {
                smart::dx12::apply(commandList, pixelHandle, pixelHash, *choice);
                // Keep the game's native draw/ExecuteIndirect, including command
                // signatures and count buffers that the generic event omits.
                return false;
            }
            ...
            // Execute the game's own draw, including DrawPrimitiveUP data.
            // A failed replacement retains the original pass to avoid stale buffers.
            return false;
        }
    }
    smart::restore(device);
...
static bool blockDispatchForCommandList(command_list* commandList)
{
	...
	// A dispatch uses the compute shader only, regardless of graphics state.
...
template<class Draw> static bool processDraw(command_list* commands, Draw&& issueDraw)
{
    ...
    // ReShade's command methods call the underlying device directly, without
    // generating a second add-on event. The backend restores shader state here.
    if (smart::other::isGL(commands->get_device()))
    {
        ...
        return false; // The native scope restores the program after the entire draw/batch.
    }
...
static bool onDispatch(command_list* commandList, uint32_t x, uint32_t y, uint32_t z)
{
    ...
    // The original add-on did not suppress direct Dispatch. Its compute groups
    // matched subsequent draw/indirect calls through the remembered compute hash.
...
static bool onDrawOrDispatchIndirect(command_list* commandList, indirect_command type, resource buffer, uint64_t offset, uint32_t count, uint32_t stride)
{
    ...
    // The native path can preserve ExecuteIndirect's binding reset semantics by
    // issuing zero commands. Fall back to the original event behavior if absent.
    if (originalMatching && smart::dx12::canHandleIndirect(commandList)) return false;
    // Native DX12 capture knows whether ExecuteIndirect also changes bindings.
    // Other APIs expose standalone typed indirect draw/dispatch commands here.
	...
	default:
		// The API cannot identify this command. Do not guess a shader stage
		// and accidentally suppress unrelated work (or an entire mixed batch).
		return false;
	}
}
...
static void onExecuteSecondary(command_list* commandList, command_list*)
{
    ...
    // Bundles may change the PSO. Wait for an explicit game binding to verify it.
    auto& data = commandList->get_private_data<CommandListDataContainer>();
...
static void onReshadePresent(effect_runtime* runtime)
{
    ...
    if (wasFinding)
    {
        // Finder keys must not also change user groups or ordinary shader hunting.
        for (const auto& group : g_toggleGroups)
...
static bool removeToggleGroups(const std::vector<int>& ids)
{
    ...
    // An unrelated deletion must not stop the managers or clear their live marks.
    // Saving below includes the current hunt's marked-shader snapshot as usual.
...
{
    // Active hunting owns live unsaved shader selections. Finish it first.
    ...
    // Build the final collection before replacing the live configuration.
...
static void displayGroupMergeControls()
{
    ...
    addonHeading("Merge toggle groups");
    {
        ...
        // Leave room for the complete group name instead of using ImGui's default item width.
...
{
    // Called after the UI's group iteration, so appending cannot invalidate it.
    ...
    // Do not duplicate the broad shader rules, startup modes or hotkey. The
    // source stays intact; only confirmed precise filters go in the new group.
...
static void displayEffectFinder(effect_runtime* runtime, ToggleGroup& group)
{
    ...
    if (addonTreeNode("Manual controls and details"))
    {
        ...
        // Refresh after a recapture clears the candidate list.
        view = g_effectFinder.view(GetTickCount64());
        if (view.stage == finder::Stage::Results && !view.candidates.empty())
        {
            if (view.quick)
...
// Stable group IDs make drops work through searches, sorting and saved filters.
// Apply only after the row loop, so no live group references are invalidated.
...
static void displayGroupState(const ToggleGroup& group,float width)
{
    const bool suspended=g_allToggleGroupsSuspended;
    const bool active=group.isActive();
    const char* text=suspended?"SUS":active?"ON":"OFF";
    ...
    const ImVec4 foreground=suspended || active?ImVec4(0.025f,0.065f,0.075f,1):ImVec4(0.90f,0.94f,1.00f,1);
    const auto pos=ImGui::GetCursorScreenPos(),size=ImVec2(width,ImGui::GetFrameHeight());
    auto* draw=ImGui::GetWindowDrawList();
    draw->AddRectFilled(pos,ImVec2(pos.x+size.x,pos.y+size.y),ImGui::GetColorU32(background),2.0f);
    if(!suspended && !active) draw->AddRect(pos,ImVec2(pos.x+size.x,pos.y+size.y),ImGui::GetColorU32(ImGuiCol_Border),2.0f);
    const auto label=ImGui::CalcTextSize(text);
    draw->AddText(ImVec2(pos.x+(size.x-label.x)*0.5f,pos.y+(size.y-label.y)*0.5f),ImGui::GetColorU32(foreground),text);
    ImGui::Dummy(size);
    if(ImGui::IsItemHovered()) ImGui::SetTooltip(suspended?"All toggle groups are suspended.":active?"Group active: its selected shaders/effects are disabled.":"Group inactive: its selected shaders/effects are allowed.");
}
...
static void displayToggleGroups(effect_runtime* runtime)
{
        ...
        for (auto& group : g_toggleGroups)
        {
            ...
            displayGroupState(group,stateWidth);
            ...
            {
                ImGui::SameLine();ImGui::SetCursorPosX(column);
                // Short counts keep the overview to one line. Full counts are in the tooltip.
        ...
        // Appending during the range loop would invalidate its group references.
...
{
    // Always reachable, even when searching the list or viewing another tab.
    for(auto& group:g_toggleGroups)
    {
        if(group.getId()==g_toggleGroupIdShaderEditing)
        {
...
static void displaySettings(reshade::api::effect_runtime* runtime)
{
    ...
    if(beginAddonTabBar())
    {
        // Cancel a pending hotkey capture when its editor is hidden by navigation.
